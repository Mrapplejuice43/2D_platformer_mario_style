from constants import BLOCK_PATH, BOTH_SIDE_TOP_GROUND_PATH, BROKEN_BLOCK_PATH, LOW_GROUND_PATH, SIDE_TOP_GROUND_LEFT_PATH, SIDE_TOP_GROUND_RIGHT_PATH, TOP_GROUND_PATH
import numpy as np
import pygame
    ...
    def __init__(self, *args, **kwargs):
        ...
        surfarrays = [
            np.array(pygame.surfarray.pixels3d(
                self.sprites[0])).transpose((1, 0, 2)),
            np.array(pygame.surfarray.pixels3d(
                self.sprites[1])).transpose((1, 0, 2)),
            np.array(pygame.surfarray.pixels3d(
                self.sprites[2])).transpose((1, 0, 2)),
            np.array(pygame.surfarray.pixels3d(
                self.sprites[3])).transpose((1, 0, 2)),
            np.array(pygame.surfarray.pixels3d(
                self.sprites[4])).transpose((1, 0, 2))
        ]
        ...
        if self.width > 1:
            ...
            for i in range(self.width - 1):
                if i != self.width - 2:
                    im_array = np.concatenate(
                        (im_array, surfarrays[0]), axis=1)
                else:
                    im_array = np.concatenate(
                        (im_array, surfarrays[3]), axis=1)
            ...
            self.sprite = pygame.image.fromstring(im_array.tobytes(), ((self.width) * 27, (self.height + 1) * 27),
                                                  "RGB")
            ...
            self.sprite = pygame.image.fromstring(im_array.tobytes(), ((self.width + 1) * 27, (self.height + 1) * 27),
                                                  "RGB").convert()
            self.sprite.set_colorkey((255, 255, 255))
